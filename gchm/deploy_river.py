...
def predict(model, args, model_weights=None,
    ...
    with torch.no_grad():
        ...
        for step, data_dict in enumerate(tqdm(dl_pred, ncols=100, desc='pred', file=sys.stdout)):  # for each training step
            ...
            # take the spatial center lon and lat from inputs for the predictions
            pred_dict['lat'].extend(list(data_dict['lat'].cpu())) # only for predict single center pixel
            pred_dict['lon'].extend(list(data_dict['lon'].cpu())) # only for predict single center pixel
            
        exclude_keys = ['lat', 'lon']
        for key in pred_dict.keys():
            print("val_dict['{}']: ".format(key), pred_dict[key])
            #print("val_dict['{}'].shape before stack: ".format(key), pred_dict[key].shape)
            if len(pred_dict[key]) >0 & (key not in exclude_keys):
                pred_dict[key] = torch.stack(pred_dict[key], dim=0)
                print("val_dict['{}'].shape after stack: ".format(key), pred_dict[key].shape)
...
if __name__ == "__main__":
    ...
    #args.model_dir = os.path.join(args.model_dir, "model_{}".format(args.model_id), args.finetune_strategy) # for fine-tuning
    args.model_dir = os.path.join(args.model_dir, "model_{}".format(args.model_id))
    ...
    if args.input_lat_lon:
        args.channels = 16  # 12 sentinel2 bands + 3 channels (lat, sin(lon), cos(lon))
    ...
    checkpoint_path = Path(args.model_dir) / 'best_weights.pt'
    ...
    # form a table storing the predictions and variances, only for predict single center pixel to a table
    recomposed_pred = pd.DataFrame()
    for k in pred_dict:
        if k == 'predictions':
            recomposed_pred['prediction'] = pred_dict[k].flatten()
        elif k == 'std':
            recomposed_pred['std'] = pred_dict[k].flatten()
        elif k == 'lat':
            recomposed_pred['lat'] = pred_dict[k].flatten()
        elif k == 'lon':
