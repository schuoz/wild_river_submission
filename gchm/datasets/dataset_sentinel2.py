...
class Sentinel2PatchesH5(Dataset):
    ...
    def _set_datasets(self):
        ...
        self.labels_mean = self.h5_file.root['wildness']
        self.labels_var = self.h5_file.root['count']
    ...
    def __getitem__(self, index):
        ...
        for k in data_dict.keys():
            print(f"Key: {k}, Shape: {data_dict[k].shape}, Dimensions: {np.ndim(data_dict[k])}")  # Debugging line
            # move channels axis as pytorch expects the shape: [batch_size, channels, height, width].
            if np.ndim(data_dict[k]) > 0:  # Only move axes for non-scalar arrays
            ...
            elif np.isscalar(data_dict[k]):  # If it's a scalar, convert it to a tensor directly
