...
class XceptionS2(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=1, num_sepconv_blocks=8, num_sepconv_filters=728, returns="targets",
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))  # Pool to 1x1 feature map
    ...
    def forward(self, x):
        ...
        if self.long_skip:
            x = x + shortcut
        print(" x.size() after long_skip: ", x.size()) # debug

        x = self.avg_pool(x)
        print('x.size() after AdaptiveAvgPool2d:', x.size())
        
        predictions = self.predictions(x)
        print('x.size() after predictions:', predictions.size())
...
def xceptionS2_08blocks_256(in_channels=16, out_channels=1, model_weights=None,
